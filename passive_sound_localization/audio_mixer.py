...
class AudioMixer:
    ...
    def get_device_indices(self):
        ...
        for i in range(num_devices):
            ...
            if device_info.get('maxInputChannels') > 0:
                ...
                logger.debug(f"Found microphone: {device_info.get('name')} at index {i}")
                if len(mic_indices) == self.config.mic_count:
                    break
        if len(mic_indices) < self.config.mic_count:
            logger.error(f"Only found {len(mic_indices)} microphones. {self.config.mic_count} required.")
    ...
    def save_audio_files(self):
        ...
        for i, frames in enumerate(self.frames_per_channel):
            output_path = os.path.join(self.output_dir_multi, f"output_channel_{i+1}.wav")
